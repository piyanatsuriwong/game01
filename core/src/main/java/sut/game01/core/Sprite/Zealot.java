...
public class Zealot {
    ...
    private  int e = 0;
    private  int offset = 0;    public Zealot(final World world,final float x,final float y){
        ...
        sprite.addCallback(new Callback<Sprite>() {
            ...
            public void onSuccess(Sprite result) {
               ...
               body = initPhysicsBody(world,
                        TestScreen.M_PER_PIXEL * x, TestScreen.M_PER_PIXEL * y);
            ...
            PlayN.keyboard().setListener(new Keyboard.Listener() {
                ...
                public void onKeyUp(Keyboard.Event event) {
                    if (event.key() == Key.RIGHT) {
                        body.applyForce(new Vec2(400f,-20f),body.getPosition());
                    }
                    if (event.key() == Key.LEFT) {
                        body.applyForce(new Vec2(-400f,-20f),body.getPosition());
                    }
    ...
    private Body initPhysicsBody(World world, float x, float y){
        ...
        fixtureDef.shape=shape; fixtureDef.density=0.4f;
        fixtureDef.friction=0.1f;
        body.createFixture(fixtureDef);
        body.setLinearDamping(0.2f);
        body.setTransform(new Vec2(x,5f),0f);
